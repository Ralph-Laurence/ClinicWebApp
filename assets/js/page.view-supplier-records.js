...
{  
    ...
    var name = tr.find(".td-supp-name").text().trim();

    var msg = `Are you sure, you want to remove "<span class="fw-bold">${name}</span>" from the list of suppliers?`;

    confirm.show(msg, "Delete Supplier", "Yes", "No", true);
}
...
{ 
    ...
    confirm.show("Are you sure, you want to remove the selected suppliers?", "Delete Suppliers", "Yes", "No");
}
...
function searchRecord()
{
    ...
    {  
        case '0': 
            $("tfoot .search-col-sup-name").find(":input").val(keyword).keyup();
            break;
        case '1': 
            $("tfoot .search-col-sup-email").find(":input").val(keyword).keyup();
            break;
        case '2': 
            $("tfoot .search-col-sup-contact").find(":input").val(keyword).keyup();
            break; 
        case '3':    
            $("tfoot .search-col-sup-id").find(":input").val(keyword).keyup();
            break;
    }
