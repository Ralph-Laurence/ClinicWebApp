...
{ 
    ...
    $('.dataset-table tfoot th').each(function (i) 
    { 
    ...
    .on("click", "tr .tr-action-edit", function()
    {
        ...
        editRecord(recordKey, rowIndex);
    })
    .on("click", "tr .tr-action-delete", function()
    {
        var tr = $(this).closest("tr");
        var recordKey = tr.find( ".record-key" ).val().trim();
        var recordTxn = tr.find( "td:eq(1)" ).text().trim();
         
        deleteRecord(recordKey, recordTxn);
    });
...
function searchRecord()
{
    ...
    switch (option)
    {
        // Patient Name
        case '0': 
            $('tfoot .search-col-patient-name .tfoot-col-search').val(keyword).keyup();
            break;

        // Checkup Number
        case '1': 
            $('tfoot .search-col-checkup-number .tfoot-col-search').val(keyword).keyup();
            break;

        // Checkup Date
        case '2': 
            $('tfoot .search-col-checkup-date .tfoot-col-search').val(keyword).keyup();
            break;

        // By Illness
        case '3': 
            $('tfoot .search-col-illness-name .tfoot-col-search').val(keyword).keyup();
            break;
    }
...
//==========================================================
//
// View details and information about the selected record
//
function showDetails(recordKey)
{
...
//
function editRecord(recordKey, rowIndex)
{
    var inputKey = $(".frm-edit #record-key").val(recordKey);
...
// Delete a record
//
function deleteRecord(recordKey, txNumber)
{
    var inputKey = $(".frm-delete #record-key").val(recordKey);

    if (System.isNullOrEmpty(inputKey))
        return; 

    var confirmMessage = `The record with Transaction Number "${txNumber}"\nwill be removed permanently.\n\n` + 
    `\u25cf Any related data associated with this record such as Prescriptions will be deleted too.\n\n Do you wish to continue?`;
...
{ 
    ...
    confirm.show("Do you really want to delete all selected records?\n\n\u25CF This will also delete any related records such as prescription information."
    + "\n\nDo you wish to continue?");
...
function notify()
{
    ...
    var errorMessage = $(".frm-session-var .error-message").val();

    if (!System.isNullOrEmpty(errorMessage))
        dialog.danger(errorMessage);
