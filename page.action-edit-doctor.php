...
require_once($rootCwd . "controllers/EditDoctorController.php");
...
<body>
    ...
    <div class="main-workarea flex-grow-1 d-flex flex-column h-100 bg-white shadow-2-strong overflow-hidden px-4 py-2">
        ...
        <div class="breadcrumbs w-100 d-flex flex-row align-items-center justify-content-start">
            ...
            <div class="breadcrumb-arrow fsz-12 fas fa-play mx-3"></div>
            <img src="assets/images/icons/bcrumb-edit-doc.png" width="20" height="20">
            <div class="ms-2 fw-bold fs-6">Edit Doctor</div>
        </div>
        <!--DIVIDER-->
        <hr class="hr" />
        <div data-simplebar class="w-100 h-100 overflow-y-auto no-native-scroll">

            <form action="<?= Tasks::EDIT_DOCTOR ?>" method="POST" id="register-form" class="overflow-hidden">
                ...
                <input type="text" name="record-key" class="record-key d-none" value="<?= $recordkey ?>">
            </form>
        </div>
        <textarea class="success-msg d-none"><?= getSuccessMessage(); ?></textarea>
        <textarea class="error-msg d-none"><?= getErrorMessage(); ?></textarea>
        <textarea class="on-cancel d-none"><?= (ENV_SITE_ROOT . Pages::DOCTORS) ?></textarea>
    </div>
    ...
    //     $_SESSION['register-patient-error']
    // );
